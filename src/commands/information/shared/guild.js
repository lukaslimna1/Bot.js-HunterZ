...
module.exports = async (guild) => {
  ...
  desc = `${desc + "❯"} **Id:** ${id}\n`;
  desc = `${desc + "❯"} **Nome:** ${name}\n`;
  desc = `${desc + "❯"} **Proprietário:** ${owner.user.username}\n`;
  desc = `${desc + "❯"} **Região:** ${preferredLocale}\n`;
  desc += "\n";
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      {
        name: `Membros do servidor [${all}]`,
        value: `\`\`\`Membros: ${users}\nBots: ${bots}\`\`\``,
        inline: true,
      },
      {
        name: `Online Stats [${onlineAll}]`,
        value: `\`\`\`Membros: ${onlineUsers}\nBots: ${onlineBots}\`\`\``,
        inline: true,
      },
      {
        name: `Categorias e canais [${totalChannels}]`,
        value: `\`\`\`Categorias: ${categories} | Texto: ${textChannels} | Voz: ${voiceChannels} | Thread: ${threadChannels}\`\`\``,
        inline: false,
      },
      {
      ...
      {
        name: "Aumentar contagem",
        value: `\`\`\`${guild.premiumSubscriptionCount}\`\`\``,
        inline: true,
      },
      {
        name: `Servidor criado [${createdAt.fromNow()}]`,
