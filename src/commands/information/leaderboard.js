...
module.exports = {
  name: "leaderboard",
  description: "exibir a tabela de classificação de XP, convites e representantes",
  ...
  slashCommand: {
    ...
    options: [
      {
        name: "type",
        description: "tipo de placar a ser exibido",
        required: true,
  ...
  async messageRun(message, args, data) {
    ...
    switch (type) {
      ...
      case "rep":
        ...
        break;
      default:
        response = "Tipo de placar inválido. Escolha `xp`, `invite`ou `rep`";
    }
...
async function getXpLeaderboard({ guild }, author, settings) {
  ...
  if (!settings.stats.enabled) return "A tabela de classificação está desativada neste servidor";

  const lb = await getXpLb(guild.id, 10);
  if (lb.length === 0) return "Não há usuários na tabela de classificação";
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `Requerido por ${author.tag}` });
