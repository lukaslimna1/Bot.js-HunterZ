...
module.exports = {
  name: "embed",
  description: "enviar mensagem incorporada",
  category: "ADMIN",
  ...
  async messageRun(message, args) {
    ...
    if (!channel) return message.reply("Forneça um canal válido");
    if (channel.type !== ChannelType.GuildText) return message.reply("Forneça um canal válido");
    if (!channel.canSendEmbeds()) {
      return message.reply("Não tenho permissão para enviar incorporações nesse canal");
    }
    message.reply(`A configuração de incorporação começou em ${channel}`);
  ...
  async interactionRun(interaction) {
    ...
    if (!channel.canSendEmbeds()) {
      return interaction.followUp("Não tenho permissão para enviar incorporações nesse canal");
    }
    interaction.followUp(`A configuração de incorporação começou em ${channel}`);
...
async function embedSetup(channel, member) {
  ...
  if (!title && !author && !description && !footer)
    return sentMsg.edit({ content: "Você não pode enviar uma incorporação vazia!", components: [] });
  ...
  await sentMsg.edit({
    content: "Adicione campos usando os botões abaixo. Clique em concluído quando terminar.",
    embeds: [embed],
