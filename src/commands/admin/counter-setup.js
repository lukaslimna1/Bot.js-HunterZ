...
module.exports = {
  ...
  async messageRun(message, args, data) {
    ...
    if (!type || !["USERS", "MEMBERS", "BOTS"].includes(type)) {
      return message.safeReply("Argumentos incorretos são passados! Tipos de contador: `usuários/membros/bots`");
    }
    if (args.length < 2) return message.safeReply("Uso incorreto! Você não forneceu o nome");
    args.shift();
