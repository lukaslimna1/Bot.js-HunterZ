...
module.exports = {
  name: "modlog",
  description: "ativar ou desativar registros de moderação",
  category: "ADMIN",
...
async function setChannel(targetChannel, settings) {
  ...
  if (targetChannel && !targetChannel.canSendEmbeds()) {
    return "Ugh! Não consigo enviar logs para esse canal? Preciso das permissões `Escrever mensagens` e `Incorporar links` nesse canal";
  }

  settings.modlog_channel = targetChannel?.id;
  await settings.save();
  return `Configuração salva! Canal Modlog ${targetChannel ? "atualizada" : "removida"}`;
}
