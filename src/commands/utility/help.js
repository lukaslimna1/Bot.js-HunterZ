...
module.exports = {
  name: "help",
  description: "menu de ajuda do comando",
  category: "UTILITY",
...
/**
* Retorna uma matriz de incorporações de mensagens para uma categoria de comando específica [MESSAGE COMMANDS]
 * @param {BotClient} client
 * @param {string} category
 * @param {string} prefix
 */
 ...
 // Para categorias RESTANTES
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new EmbedBuilder()
      ...
      .setDescription(item.join("\n"))
      .setFooter({
        text: `page ${index + 1} of ${arrSplitted.length} | Digite ${prefix}help <command> para obter mais informações sobre comandos`,
      });
    arrEmbeds.push(embed);
  });
